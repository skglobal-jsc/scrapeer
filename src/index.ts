      ...
      switch (tagName) {
        ...
        case 'table':
          ...
          if (table_result.totalRows == 1 && table_result.totalCols == 1) {
            ...
            let tableDesc = getTableDescription(table_result,lang);
    ...
    for (let j = 0; j < $element.children.length; j++) {
      ...
      switch (tagName) {
        ...
        case 'a':
          ...
          let text_a = '';
          if (child.children && child.children.length > 0) {
            text_a = parseParagraphForRAGT($, child, item,lang);
          }
          ...
          break;
        case 'span':
          const desc_span = parseParagraphForRAGT($, child, item,lang);
          ...
          break;
        case 'h2':
          let text_h2 = parseParagraphForRAGT($, child, item,lang).trim();
          if (text_h2) {
            description += '●' + text_h2 + '\n';
          }
          break;
        ...
        case 'h4':
        case 'h5':
          let text_h = parseParagraphForRAGT($, child, item,lang).trim();
          if (text_h) {
            description += text_h + '\n';
          }
        ...
        case 'article':
        case 'address':
          let text_component = parseParagraphForRAGT($, child, item,lang).trim();
          if (text_component) {
            description += '\n' + text_component + '\n';
          }
          break;
        case 'h6':
          let text_h6 = parseParagraphForRAGT($, child, item,lang).trim();
          if (text_h6) {
            description += text_h6 + '\n';
          }
          break;
        case 'strong':
          const desc_strong = parseParagraphForRAGT($, child, item,lang);
          ...
          break;
        case 'div':
          const desc_div = parseParagraphForRAGT($, child, item,lang);
          ...
          break;
        case 'p':
          const desc_p = parseParagraphForRAGT($, child, item,lang);
...
const extractTextFromDomForRAGT = ($: any, $el: any, item: IArticle, lang:string = 'ja') => {
  let description = '';
  if (!$el) return '';
...
const generateDescriptionFromDomForRAGT = (
  $: any,
  item: IArticle,
  contentSector: string = 'body',
  titleEle?: any,
  ...
  let description = cleanText(extractTextFromDomForRAGT($clone, $content, item,_lang));
  // description =
  //   'ここから本文です。' + '\n\n' + description + '\n\n' + '以上です。';
  description =
    useLocale('StartArticle',_lang) + '\n\n' + description + '\n\n' + useLocale('EndArticle',_lang);
